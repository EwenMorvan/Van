...
static void eth_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    switch (event_id) {
        ...
        case ETHERNET_EVENT_DISCONNECTED:
            ...
            REPORT_ERROR(MAIN_PCB_ERR_ETH_DISCONNECTED, TAG, "Ethernet disconnected", 0);
...
static esp_err_t configure_static_ip(void)
{
    ...
    if (ret != ESP_OK && ret != ESP_ERR_ESP_NETIF_DHCP_ALREADY_STOPPED) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to stop DHCP client", 0);
        return ret;
    }
    ...
    if (ret != ESP_OK) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to set static IP", 0);
        return ret;
    }
...
static void wait_for_ip_configuration(void)
{
    ...
    REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "IP configuration timeout", 0);
...
static void udp_receive_task(void *pvParameters)
{
    ...
    if (!(bits & IP_READY_BIT)) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "IP configuration wait timeout", 0);
    }
    ...
    if (udp_socket < 0) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to create UDP socket", errno);
    ...
    if (bind(udp_socket, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to bind UDP socket", errno);
        close(udp_socket);
        udp_socket = -1;
        if (ethernet_ready_semaphore != NULL) {
            xSemaphoreGive(ethernet_ready_semaphore);
    ...
    while (1) {
        ...
        if (len > 0) {
            ...
            REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to receive UDP packet", errno);
            break;
        }
...
esp_err_t ethernet_manager_init(const ethernet_config_t *config)
{
    if (config == NULL) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_INVALID_ARG, TAG, "Invalid Ethernet configuration (NULL)", 0);
    ...
    if (eth_event_group == NULL) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to create event group", 0);
        return ESP_FAIL;
    }
    ...
    if (ethernet_ready_semaphore == NULL) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to create ready semaphore", 0);
        return ESP_FAIL;
    }
    ...
    if (ret != ESP_OK) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to initialize netif", 0);
        vEventGroupDelete(eth_event_group);
    ...
    if (ret != ESP_OK) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to create event loop", 0);
        vEventGroupDelete(eth_event_group);
        vSemaphoreDelete(ethernet_ready_semaphore);
        return ret;
    }
    ...
    if (eth_netif == NULL) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to create network interface", 0);
        vEventGroupDelete(eth_event_group);
        vSemaphoreDelete(ethernet_ready_semaphore);
        return ESP_FAIL;
    }
    ...
    if (ret != ESP_OK) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "SPI bus initialization failed", 0);
        vEventGroupDelete(eth_event_group);
        vSemaphoreDelete(ethernet_ready_semaphore);
        return ret;
    }
    ...
    if (ret != ESP_OK) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "SPI device addition failed", 0);
    ...
    if (mac == NULL) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to create MAC instance", 0);
    ...
    if (phy == NULL) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Failed to create PHY instance", 0);
    ...
    if (ret != ESP_OK) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Ethernet driver installation failed", 0);
    ...
    if (ret != ESP_OK) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Ethernet start failed", 0);
    ...
    if (!(bits & ETH_CONNECTED_BIT)) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Ethernet connection timeout", 0);
        esp_eth_driver_uninstall(eth_handle);
        vEventGroupDelete(eth_event_group);
        vSemaphoreDelete(ethernet_ready_semaphore);
    ...
    if (xSemaphoreTake(ethernet_ready_semaphore, pdMS_TO_TICKS(10000)) == pdTRUE) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Ethernet initialization timeout - UDP socket not ready", 0);
        esp_eth_driver_uninstall(eth_handle);
        vEventGroupDelete(eth_event_group);
        vSemaphoreDelete(ethernet_ready_semaphore);
        return ESP_ERR_TIMEOUT;
...
esp_err_t ethernet_send(const uint8_t *data, uint32_t length, const char *dest_ip, uint16_t dest_port)
{
    if (data == NULL || length == 0) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Invalid send parameters", 0);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (udp_socket < 0) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Socket not ready", 0);
        return ESP_ERR_INVALID_STATE;
    }

    if (!ethernet_is_connected()) {
        // ESP_LOGE(TAG, "Ethernet not connected");
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Ethernet not connected", 0);
        return ESP_ERR_INVALID_STATE;
    }
    ...
    if (dest_addr.sin_addr.s_addr == INADDR_NONE) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Invalid destination IP", dest_ip);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (sent < 0) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Send failed", errno);
        return ESP_FAIL;
    }
...
esp_err_t ethernet_send_broadcast(const uint8_t *data, uint32_t length, uint16_t dest_port)
{
    ...
    if (sent < 0) {
        ...
        REPORT_ERROR(MAIN_PCB_ERR_COMM_FAIL, TAG, "Broadcast send failed", errno);
        return ESP_FAIL;
    }
